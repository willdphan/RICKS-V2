...
import "test/utils/mocks/BatchERC721.sol";
import "test/utils/mocks/MockRecipients.sol";
import "test/utils/mocks/MockCheckpointedERC721.sol";
import "test/utils/interfaces/IBatchERC721.sol";
import {DSTestPlus} from "solmate/test/utils/DSTestPlus.sol";

abstract contract BatchERC721SpecTest is DSTestPlus, ERC721TokenReceiver {
    mapping(uint256 => bool) public uniques;
    uint256[] newarr;
    IBatchERC721 public token;

    function setUp() public virtual {}

    function uniquify(uint256[] memory input)
        public
        returns (uint256[] memory ret)
    {
        ...
        for (uint256 i; i < newarr.length; i++) delete uniques[newarr[i]];
        return newarr;
    }
...
contract CheckpointedERC721Test is BatchERC721SpecTest {
    function setUp() public override {
        super.setUp();
        token = IBatchERC721(address(new MockCheckpointed("Token", "TKN")));
    }
}

contract BatchERC721Test is BatchERC721SpecTest {
    function setUp() public override {
        super.setUp();
        token = IBatchERC721(address(new MockBatchERC721("Token", "TKN")));
    }
}
